...
const COLUMNS = [
  ...
  "total_doses", "coverage_pct", "aefi_minor", "aefi_serious",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [rows] = await bq.query({
      query: `SELECT * FROM \`${DATASET}.v_ward_cumulative\` ORDER BY local_level_name, ward_no`,
      location: LOCATION,
    });
    ...
    res.status(200).send(toCsv(flat, COLUMNS));
  } catch (err) {
